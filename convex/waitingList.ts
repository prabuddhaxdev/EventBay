...
/**
 * Query: get a user's position in the waiting list.
 */
...
/**
 * Internal mutation: process the waiting list and offer tickets.
 */
export const processQueue = internalMutation({
  ...
  handler: async (ctx, { eventId }) => {
    ...
    // Offer tickets to next waiting users
...
/**
 * Internal mutation: expire a single offer and process queue.
 */
...
/**
 * Internal mutation: cleanup expired offers in bulk.
 */
export const cleanupExpiredOffers = internalMutation({
  ...
  handler: async (ctx) => {
    ...
    for (const [eventId, offers] of Object.entries(grouped)) {
      await Promise.all(
        offers.map((offer) =>
          ctx.db.patch(offer._id, { status: WAITING_LIST_STATUS.EXPIRED })
        )
      );
...
/**
 * Public mutation: release a ticket manually.
 */
